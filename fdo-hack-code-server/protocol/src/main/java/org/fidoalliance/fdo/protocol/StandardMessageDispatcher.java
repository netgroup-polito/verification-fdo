...
public class StandardMessageDispatcher implements MessageDispatcher {
  ...
  protected void doTo1ProveRv(DispatchMessage request, DispatchMessage response)
    ...
    System.out.println("MY CODE STARTS HERE TO1");

    To1dPayload to1dPayload = new To1dPayload();
    //3=http or 5=https
    String defaultBob = "[[\"127.0.0.1\",\"host.docker.internal\",%s,3]]";
    String defaultPort = Config.getWorker(HttpServer.class).getHttpPort();
    String rviString = String.format(defaultBob, defaultPort);
    System.out.println("DEFAULT PORT " + defaultPort);

    to1dPayload.setAddressEntries(
            Mapper.INSTANCE.readJsonValue(rviString, To2AddressEntries.class));

    System.out.println("MY CODE STARTS HERE -333333");
    Hash to0dHash = new Hash();
    to1dPayload.setTo1ToTo0Hash(to0dHash);
    System.out.println("MY CODE STARTS HERE -222222");
    ...
    System.out.println("MY CODE STARTS HERE -111111");
    String alias = KeyResolver.getAlias(PublicKeyType.SECP256R1, KeySizeType.SIZE_384);
    //SECP384R1
    System.out.println("MY CODE STARTS HERE 00000");
    OwnerPublicKey resellerPublicKey = getCryptoService().encodeKey(PublicKeyType.SECP256R1,
            PublicKeyEncoding.X509,
            resolver.getCertificateChain(alias));
    System.out.println("MY CODE STARTS HERE 1111");
    PrivateKey resellerPrivateKey = resolver.getPrivateKey(alias);

    System.out.println("MY CODE STARTS HERE 2222");

    CryptoService cs = Config.getWorker(CryptoService.class);

    CoseSign1 signReseller = null;
    try {
      signReseller = cs.sign(
              Mapper.INSTANCE.writeValue(to1dPayload),
              resellerPrivateKey,
              resellerPublicKey
      );
    ...
    response.setMessage(Mapper.INSTANCE.writeValue(signReseller));
  }
  ...
  protected void doHelloDevice(DispatchMessage request, DispatchMessage response)
    ...
    System.out.println("MY CODE STARTS HERE TO2");
    ...
    String alias = KeyResolver.getAlias(PublicKeyType.SECP384R1, KeySizeType.SIZE_384);
    OwnerPublicKey resellerPublicKey = getCryptoService().encodeKey(PublicKeyType.SECP384R1,
            PublicKeyEncoding.X509,
            resolver.getCertificateChain(alias));
    PrivateKey resellerPrivateKey = resolver.getPrivateKey(alias);
    //PrivateKey privateKey = resolver.getPrivateKey(getCryptoService().decodeKey(ownerPublicKey));
    uph.setOwnerPublicKey(resellerPublicKey);
    ...
    try {
      ...
      CoseSign1 sign1 = cs.sign(payload, resellerPrivateKey, resellerPublicKey);
      sign1.setUnprotectedHeader(uph);
      response.setMessage(Mapper.INSTANCE.writeValue(sign1));
    } finally {
      cs.destroyKey(resellerPrivateKey);
    }
  ...
  protected void doProveOwnerHeader(DispatchMessage request, DispatchMessage response)
    ...
    System.out.println("OWNER CORRECTLY SIGNED NONCE");
    ...
    System.out.println("HERE VERIFICATION 2");
